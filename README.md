...
- 支持展开/折叠查看每个哨位的人员列表
- 高亮显示被多个哨位共享的人员
- 清晰标识人员来源（自动提取、临时添加、临时移除）

**临时性人员调整**
- 为指定哨位临时添加人员（仅在本次排班中生效）
- 为指定哨位临时移除人员（仅在本次排班中生效）
- 使用图标和颜色区分临时更改
- 支持撤销单个哨位或所有哨位的临时更改
- 临时更改在排班完成或取消后自动丢弃
...
**手动添加参与人员**
- 添加不属于任何哨位的人员参与排班
- 这些人员可以被分配到任何哨位（对所有哨位可用）
- 适用于机动人员、管理人员、临时支援人员

**两种添加方式对比**

| 添加方式 | 操作位置 | 关联哨位 | 可保存为永久 | 排班算法中的可见性 |
|---------|---------|---------|-------------|------------------|
| 为哨位临时添加 | 哨位卡片中 | 特定哨位 | ✅ 可以 | 只对该哨位可用 |
| 手动添加参与人员 | 手动添加区域 | 无 | ❌ 不可以 | 对所有哨位可用 |
