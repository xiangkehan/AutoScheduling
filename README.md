...
> **注意**：步骤2和步骤3的顺序已调整，现在先选择哨位，再基于哨位智能管理人员。
...
将临时调整保存到数据库：
- **保存为永久**：将哨位的临时人员更改永久保存到数据库
- **确认对话框**：保存前显示详细的更改列表（添加/移除的人员）
- **数据库更新**：通过 `IPositionService.UpdateAsync` 更新哨位的 `AvailablePersonnelIds`
- **本地同步**：保存成功后自动更新本地数据
- **错误处理**：保存失败时显示友好的错误提示，保留临时更改状态

##### 手动添加参与人员

支持添加不属于任何哨位的人员：
- **灵活添加**：从所有可用人员中选择不在任何已选哨位列表中的人员
- **全局可用**：这些人员可以被分配到任何哨位（对所有哨位可用）
- **适用场景**：机动人员、管理人员、临时支援人员、特殊角色人员
- **独立管理**：在"手动添加的人员"区域单独显示和管理
- **不可永久保存**：因为没有关联到具体哨位，无法保存为永久

##### 两种添加方式对比

| 添加方式 | 操作位置 | 人员来源 | 关联哨位 | 可保存为永久 | 排班算法中的可见性 |
|---------|---------|---------|---------|-------------|------------------|
| **为哨位临时添加** | 哨位卡片中 | 所有可用人员 | 特定哨位 | ✅ 可以 | 只对该哨位可用 |
| **手动添加参与人员** | 手动添加区域 | 不在任何哨位列表中的人员 | 无 | ❌ 不可以 | 对所有哨位可用 |

##### 核心管理器：PositionPersonnelManager

系统引入了 `PositionPersonnelManager` 类来管理哨位与人员的临时关联：
- **状态跟踪**：维护原始数据和当前数据（包含临时更改）
- **更改记录**：记录每个哨位的添加和移除操作
- **撤销支持**：提供单个哨位或全部哨位的撤销功能
- **数据隔离**：临时更改不影响数据库，直到用户选择保存为永久

##### 使用流程示例

1. **选择哨位**（步骤2）：选择"大门岗"、"后门岗"等哨位
2. **自动提取**（步骤3）：系统自动提取这些哨位的可用人员
3. **临时调整**：
   - 为"大门岗"临时添加"赵六"（从所有可用人员中选择）
   - 为"大门岗"临时移除"孙七"
4. **手动添加**：添加"郑十"（管理人员）到参与人员，对所有哨位可用
5. **保存为永久**（可选）：将"大门岗"的临时更改保存到数据库
6. **继续排班**：进入步骤4配置约束，完成排班
