...
- **哨位排班系统（Guard_Duty_Scheduling_System）**: 完整的自动化排班管理系统
- **哨位（Position）**: 需要人员值守的具体位置或岗位
- **人员（Personnel）**: 可以被分配到哨位值班的工作人员
- **技能（Skill）**: 人员具备的专业能力或资质
- **排班表（Schedule）**: 包含多个单次排班的完整调度方案
- **单次排班（Single_Shift）**: 特定哨位在特定时间段的人员分配
- **硬约束（Hard_Constraint）**: 必须满足的调度规则
- **软约束（Soft_Constraint）**: 用于优化调度质量的评分规则
- **可行张量（Feasibility_Tensor）**: 三维二进制数组，表示哨位-时段-人员分配的可行性
- **历史缓冲区（History_Buffer）**: 临时存储未确认排班结果的数据区域
- **确定历史区（Confirmed_History）**: 存储已确认实施的排班记录的数据区域

## 需求

### 需求 1

**用户故事:** 作为排班管理员，我希望能够管理哨位信息，以便为每个哨位配置正确的要求和描述。
...
**用户故事:** 作为排班管理员，我希望能够管理人员信息和技能，以便系统能够根据人员能力进行合理分配。
...
**用户故事:** 作为排班管理员，我希望系统能够生成排班表，以便为所有哨位安排合适的人员值班。
...
**用户故事:** 作为排班管理员，我希望系统能够管理排班历史，以便跟踪和确认排班实施情况。
...
**用户故事:** 作为排班管理员，我希望系统能够应用硬约束条件，以便确保排班方案的合规性和可行性。
...
**用户故事:** 作为排班管理员，我希望系统能够应用软约束评分，以便优化排班质量和人员工作负荷平衡。
...
**用户故事:** 作为排班管理员，我希望系统能够高效处理大规模排班计算，以便在合理时间内生成排班方案。
